...
public:
  ...
  void setup_inverse()
  {
    ...
    auto jac0_0 = 0 - (steady_state ? 0 : r4c1.get_gradient()) - r5.get_gradient() - (steady_state ? 0 : c2.get_gradient());
    jacobian << jac0_0;
...
bool iterate() const
{
   ...
   auto  i0_= input_state[0];
    ...
    auto eq0 = - (steady_state ? 0 : r4c1.get_current(i0_, d0_)) - r5.get_current(s0_, d0_) - (steady_state ? 0 : c2.get_current(s0_, d0_));
    eqs << eq0;
